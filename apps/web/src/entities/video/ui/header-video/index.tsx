				...
				<StreamersContainer>
					{
						videos.map((video, index) => (
							<Streamer key={index} isActive={index === activeVideo} onClick={() => setActiveVideo(index)} src={video.profileImage}/>
						))
					}
				</StreamersContainer>
			</LiveInfoContainer>
...
const Streamer = styled.img<{isActive: boolean}>`
    ...
    border: ${({ theme, isActive }) => isActive ? `2px solid ${theme.colors.primary.normal}` : "none"};
